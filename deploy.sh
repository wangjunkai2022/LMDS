...
EOF
  ...
  "iproyal")
    ...
    exitstatus=$?
    if [ $exitstatus = 0 ] && [ -z "$iproyalemail" ]; then
      echo -e "\e[36;1mCancel - Container not created\e[0m"
    else
      #echo $honeyemail $honeypass $honeyname
      if [ -d services/iproyal ]; then
...
EOF
  ...
  "peer2profit")
    ...
    if [ $exitstatus = 0 ]; then
      if [ -d services/peer2profit ]; then
